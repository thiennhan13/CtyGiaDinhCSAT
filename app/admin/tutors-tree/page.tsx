...
export default async function TutorsTreePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-foreground">Sơ đồ Gia sư — Lớp học</h2>
        <p className="text-sm text-muted-foreground mt-1">Tổng quan toàn bộ gia sư đang hoạt động và các lớp họ đang phụ trách</p>
      </div>
      ...
      <div className="grid gap-4">
        {!tutors || tutors.length === 0 ? (
          ...
          tutors.map((tutor) => {
            ...
            return (
              <div key={tutor.tutor_id} className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                {/* Tutor Header */}
                <div className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-border">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-primary font-bold text-sm shrink-0 select-none">
                      {tutor.name?.substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <Link href={`/admin/tutors/${tutor.tutor_id}`} className="font-bold text-foreground hover:text-primary transition-colors text-base">
                        {tutor.name}
                      </Link>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <Link
                      ...
                      className="text-xs font-semibold text-primary hover:text-blue-800 flex items-center gap-1"
                    >
                ...
                {activeClasses.length === 0 ? (
                  <div className="px-5 py-4 text-sm text-muted-foreground/70 italic">Chưa có lớp nào đang hoạt động</div>
                ) : (
                  <div className="divide-y divide-slate-100">
                    {activeClasses.map((cls: any) => {
                      ...
                      return (
                        ...
                        >
                          {/* Tree connector */}
                          <div className="flex items-center gap-2 text-muted-foreground/40 shrink-0 ml-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {studentNames.length > 0 && (
                              <div className="flex items-center gap-1 mt-0.5 ml-5">
                                ...
                                <span className="text-xs text-muted-foreground/70 truncate">{studentNames.join(', ')}</span>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            {cls.end_date && (
                              <span className={`text-xs ${isExpiring ? 'text-amber-600 font-semibold' : 'text-muted-foreground/70'}`}>
                                HH: {cls.end_date}
                              </span>
                            )}
                ...
                {inactiveClasses.length > 0 && (
                  <div className="px-5 py-2.5 border-t border-dashed border-border/60 bg-secondary/50/50">
                    <span className="text-xs text-muted-foreground/70">+ {inactiveClasses.length} lớp đã kết thúc (ẩn)</span>
                  </div>
                )}
              </div>
            );
