...
export default function DashboardPage() {
  ...
  const [expiringClasses, setExpiringClasses] = useState<any[]>([]);
  ...
  useEffect(() => {
    async function loadKpis() {
      const { count: classCount } = await supabase
        .from('classes').select('class_id', { count: 'exact', head: true }).eq('status', 'active');
      setActiveClassCount(classCount || 0);

      const { count: unpaid } = await supabase
        .from('payments').select('payment_id', { count: 'exact', head: true }).eq('status', 'unpaid');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className={`border shadow-none ${expiringClasses.length > 0 ? 'border-red-300 bg-red-50' : 'border-slate-200'}`}>
          <CardContent className="p-5">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 flex items-center gap-1">
            ...
            {expiringClasses.slice(0, 2).map(c => (
              <Link key={c.class_id} href={`/admin/classes/${c.class_id}`} className="block text-[10px] text-red-600 truncate hover:underline">
                {c.name} — {c.end_date}
              </Link>
            ))}
          </CardContent>
            ...
            {loadingSessions ? (
              ...
              sessionForSelected
               ...
               .map((session, idx) => {
                 ...
                 return (
                 // F2: Click lịch → navigate đến lớp
